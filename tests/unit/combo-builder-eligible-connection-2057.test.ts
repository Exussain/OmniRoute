...
test("forced combo picker includes failed enabled connections but not disabled ones", () => {
  const failed = { id: "failed", testStatus: "error", isActive: true };
  const disabled = { id: "disabled", testStatus: "error", isActive: false };
  const active = { id: "active", testStatus: "active", isActive: true };

  assert.deepEqual(resolveComboModelPickerConnections([active], [active, failed, disabled], true), [
    active,
    failed,
  ]);
});
