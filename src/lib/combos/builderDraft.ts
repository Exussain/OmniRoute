...
/**
 * Resolve the connection list used by the legacy combo model picker.
 * Force mode intentionally bypasses only test-status filtering; explicitly
 * disabled connections remain unavailable.
 */
export function resolveComboModelPickerConnections<T extends ComboEligibleConnectionLike>(
  activeConnections: T[] = [],
  allConnections: T[] = [],
  forceIncludeFailed: boolean = false
): T[] {
  if (!forceIncludeFailed) return activeConnections;
  return allConnections.filter((connection) => connection.isActive !== false);
}
