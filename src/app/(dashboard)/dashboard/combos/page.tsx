...
import {
  ...
  parseQualifiedModel,
  resolveComboModelPickerConnections,
  ...
  const fetchData = async () => {
    try {
      ...
      if (providersRes.ok) {
        ...
        setAllProviderConnections(connections);
        const active = connections.filter(isEligibleActiveConnection);
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <ComboFormModal
        ...
        onSave={handleCreate}
        activeProviders={activeProviders}
        allProviders={allProviderConnections}
        combo={null}
...
function ComboFormModal({
  ...
  const [forceIncludeFailedModels, setForceIncludeFailedModels] = useState(false);
  ...
  const failedProviderCount = allProviders.filter(
    (connection) => connection?.isActive !== false && !isEligibleActiveConnection(connection)
  ).length;
  const modelPickerProviders = resolveComboModelPickerConnections(
    activeProviders,
    allProviders,
    forceIncludeFailedModels
  );
  ...
  useEffect(() => {
    if (!isOpen) return;
    setForceIncludeFailedModels(false);
  ...
  return (
    <>
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      ...
                      {failedProviderCount > 0 && (
                        <button
                          type="button"
                          onClick={() => {
                            setForceIncludeFailedModels(true);
                            setShowModelSelect(true);
                          }}
                          className="text-[10px] font-medium text-amber-700 dark:text-amber-300 hover:text-amber-600 dark:hover:text-amber-200 transition-colors"
                          title={getI18nOrFallback(
                            t,
                            "forceEnableFailedModelsHint",
                            "Add models from active providers whose latest connection test failed."
                          )}
                        >
                          {getI18nOrFallback(
                            t,
                            "forceEnableFailedModels",
                            "Force-enable failed models"
                          )}
                        </button>
                      )}
                    </div>
      ...
      <ModelSelectModal
        ...
        activeProviders={modelPickerProviders}
